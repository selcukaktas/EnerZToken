...
contract('EnerZToken', function(accounts) {
  ...
  it('initializes the contract with the correct values', function() {
    return EnerZToken.deployed().then(function(instance) {
      ...
      assert.equal(name, 'EnerZ Token', 'Tamam: has the correct name');
      return tokenInstance.symbol();
    }).then(function(symbol) {
      assert.equal(symbol, 'EnerZ', 'Tamam: has the correct symbol');
    })
  })
  ...
  it('transfers token ownership', function() {
    return EnerZToken.deployed().then(function(instance) {
      ...
      return tokenInstance.transfer.call(accounts[1], 5); //99999999999999999999999
    }).then(assert.fail).catch(function(error) {
      assert(error.message.indexOf('revert') >= 0, 'Hata Mesaji: error message must contain revert');
      return tokenInstance.transfer.call(accounts[1], 5, { from: accounts[2] });
    }).then(function(success) {
      assert.equal(success, true, 'Başarılı: it returns true');
