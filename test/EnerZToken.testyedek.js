...
contract('EnerZToken', function(accounts) {
  ...
  it('transfers token ownership', function() {
    return EnerZToken.deployed().then(function(instance) {
      ...
      return tokenInstance.transfer(accounts[1], 5, { from: accounts[2] });
      ...
      assert.equal(balance.toNumber(), 5, 'adds the amount to the receiving account');
  ...
  it('approves tokens for delegated transfer', function() {
    return EnerZToken.deployed().then(function(instance) {
      ...
      return tokenInstance.approve(accounts[1], 5, { from: accounts[2] });
    }).then(function(receipt) {
      assert.equal(receipt.logs.length, 1, 'triggers one event');
      assert.equal(receipt.logs[0].event, 'Approval', 'should be the "Approval" event');
      assert.equal(receipt.logs[0].args._owner, accounts[2], 'hata mesaji: logs the account the tokens are authorized by');
      assert.equal(receipt.logs[0].args._spender, accounts[1], 'logs the account the tokens are authorized to');
      assert.equal(receipt.logs[0].args._value, 5, 'logs the transfer amount');
      return tokenInstance.allowance(accounts[2], accounts[1]);
    }).then(function(allowance) {
      assert.equal(allowance.toNumber(), 5 , 'stores the allowance for delegated trasnfer');
    });
  });
